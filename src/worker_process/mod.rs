...
mod tests {
    ...
    #[expect(
        clippy::expect_used,
        reason = "test setup helper; a failure staging the stand-in worker should fail the test"
    )]
    fn write_script(dir: &Utf8Path, body: &str) -> camino::Utf8PathBuf {
        let script = dir.join("fake_worker");
        std::fs::write(script.as_std_path(), body).expect("write worker script");
        let mut perms = std::fs::metadata(script.as_std_path())
            .expect("stat script")
            .permissions();
        perms.set_mode(0o755);
        std::fs::set_permissions(script.as_std_path(), perms).expect("chmod script");
        script
    }

    #[expect(
        clippy::expect_used,
        reason = "test setup helper; a failure preparing the temp worker should fail the test"
    )]
