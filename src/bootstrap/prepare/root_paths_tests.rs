//! Settings-path resolution: the `PG_EMBED_ROOT` contract and the Unix
//! permission rules.
//!
//! Extracted from `tests.rs`, which the two features together pushed past the
//! suite's module-size limit. The three modules stay together because the
//! platform-specific pair share `embed_root_cases`, which states the contract
//! once so both resolvers are held to it.
...
mod unix_tests {
    ...
    use super::*;
    use crate::{
        bootstrap::prepare::unix_user::ensure_pgpass_for_user,
        privileges::default_paths_for,
    };
