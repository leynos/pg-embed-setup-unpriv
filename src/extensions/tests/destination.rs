...
///
/// Arrangement can fail, so it is propagated rather than unwrapped: only a
/// test body may treat a failure as the verdict.
fn tree() -> Result<(tempfile::TempDir, InstallTree, Utf8PathBuf)> {
    let (temp, root) = temp_root()?;
    let tree = InstallTree::open(&root)
        .map_err(|err| eyre!("cannot open the installation tree {root}: {err}"))?;
    Ok((temp, tree, root))
}
