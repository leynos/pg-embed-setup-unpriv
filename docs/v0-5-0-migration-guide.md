...
- Keep `rstest-bdd` and `rstest-bdd-macros` aligned at `0.5.0` when running
  behavioural tests.
...
### `rstest-bdd` 0.5.0 step macro contract

The v0.5.0 `#[given]`, `#[when]`, and `#[then]` attributes continue to support
the direct string form used by this repository:
...
The macros also continue to support `expr = "..."` for compatibility, but the
direct string form remains the preferred form.

Fixtures are injected by parameter name. A step parameter named `world` is
resolved from the `world` fixture without adding `fixtures` metadata to the
attribute invocation. Use `#[from(name)]` only when a parameter needs to refer
to a fixture under a different name.

The internal `step!(@pattern ...)` macro arm now carries fixture metadata and
execution mode. Those are generated by `rstest-bdd-macros` and consumed by the
matching `rstest-bdd` runtime crate; test code must not pass `fixtures` or
`mode` arguments by hand. A mixed dependency state, for example `rstest-bdd`
0.5.0 with `rstest-bdd-macros` 0.3.2, can leave old macro expansions calling
the new runtime macro with too few arguments.

PR `#122` is unblocked once both crates and `rstest-bdd-patterns` resolve to
`0.5.0` and the BDD suite passes without `@pattern`/`missing tokens` macro
errors.
